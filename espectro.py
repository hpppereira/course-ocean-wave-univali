...
def espec2(x,y,dt,gl,han):
    ...
    #                         col 7: espectro de coerencia (arrumar)
    ...
    mat1xre=np.zeros((l,c)) #matriz com valores reais da fft de x
    mat1xim=np.zeros((l,c)) #matriz com valores imaginarios da fft de x
    mat1yre=np.zeros((l,c)) #matriz com valores reais da fft de y
    mat1yim=np.zeros((l,c)) #matriz com valores imaginarios da fft de x

    mat2xre=np.zeros((l,1)) #vetor com valores reais da fft de x
    mat2xim=np.zeros((l,1)) #vetor com valores imaginarios da fft de x
    mat2yre=np.zeros((l,1)) #vetor com valores reais da fft de y
    mat2yim=np.zeros((l,1)) #vetor com valores imaginarios da fft de x
    ...
    for i in range(c):
        ...
        if han==1:
            ...
            #separa componentes reais e imaginarias
            mat1xre[:,i]=np.real(fftx)
            mat1xim[:,i]=np.imag(fftx)

            #calcula o espectro para cada coluna
            mat1x[:,i]=2*dt*(mat1xre[:,i]*mat1xre[:,i]+mat1xim[:,i]*mat1xim[:,i])/(N*0.375) #0.375 eh a correcao da janela
            ...
            #separa componentes reais e imaginarias
            mat1yre[:,i]=np.real(ffty)
            mat1yim[:,i]=np.imag(ffty)

            #calcula o espectro para cada coluna
            mat1y[:,i]=2*dt*(mat1yre[:,i]*mat1yre[:,i]+mat1yim[:,i]*mat1yim[:,i])/(N*0.375) #0.375 eh a correcao da janela

            # SERIES CRUZADAS

            #calcula o co-esoectro para cada coluna
            mat1co[:,i]=2*dt*(mat1xre[:,i]*mat1yre[:,i]+mat1xim[:,i]*mat1yim[:,i])/(N*0.375)

            #calcula o quad-espectro para cada coluna
            mat1qd[:,i]=2*dt*(mat1xim[:,i]*mat1yre[:,i]-mat1xre[:,i]*mat1yim[:,i])/(N*0.375)
        ...
        #janela retangular                
            ...
            #calcula o espectro para cada coluna
            mat1x[:,i]=2*dt*(mat1xre[:,i]*mat1xre[:,i]+mat1xim[:,i]*mat1xim[:,i])/N #0.375 eh a correcao da janela
            ...
            #calcula o espectro para cada coluna
            mat1y[:,i]=2*dt*(mat1yre[:,i]*mat1yre[:,i]+mat1yim[:,i]*mat1yim[:,i])/N

            # SERIES CRUZADAS

            #calcula o co-espectro para cada coluna
            mat1co[:,i]=2*dt*(mat1xre[:,i]*mat1yre[:,i]+mat1xim[:,i]*mat1yim[:,i])/N

            #calcula o quad-espectro para cada coluna
            mat1qd[:,i]=2*dt*(mat1xim[:,i]*mat1yre[:,i]-mat1xre[:,i]*mat1yim[:,i])/N
    ...
    for i in range(l):
        ...
        #mat2xre[i,0]=np.mean(mat1xre[i,:])
        #mat2xim[i,0]=np.mean(mat1xim[i,:])
        #mat2yre[i,0]=np.mean(mat1yre[i,:])
        #mat2yim[i,0]=np.mean(mat1yim[i,:])

    #calculo do co-espectro
    #mat2co[:,0]=2*dt*(mat2xre[:,0]*mat2yre[:,0]+mat2xim[:,0]*mat2yim[:,0])/N

    #calculo do quad-espectro
    #mat2qd[:,0]=2*dt*(mat2xim[:,0]*mat2yre[:,0]-mat2xre[:,0]*mat2yim[:,0])/N

    #calculo da amplitude do espectro cruzado
    mat2amp[:,0]=np.sqrt(mat2qd[:,0]**2+mat2co[:,0]**2)

    #calculo do espectro de fase
    mat2fase[:,0]=np.arctan(mat2qd[:,0]/mat2co[:,0])*180/np.pi

    #calculo do espectro de coerencia
    mat2coer[:,0] = (mat2co[:,0]**2+mat2qd[:,0]**2)/(mat2x[:,0]*mat2y[:,0])
